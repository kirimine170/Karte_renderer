...
func TestParseFormatManifestRejectsUnsafePaths(t *testing.T) {
	tests := []struct {
		...
		{name: "data URL", path: "data:text/css,body{}", want: "package-relative local path"},
		{name: "mailto URL", path: "mailto:style.css", want: "package-relative local path"},
		{name: "URN", path: "urn:karte:style", want: "package-relative local path"},
